...
export function UdemyCouponActions({ coupon }: { coupon: UdemyCouponFormValue }) {
  ...
  async function handleToggle() {
    ...
    await fetch(`/api/admin/udemy/${coupon.id}`, {
      method: "PATCH",
      headers: {
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/admin/udemy/${coupon.id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    if (!response.ok) {
      setError("Nie udało się zapisać kodu. Sprawdź pola i unikalność courseId + locale + couponCode.");
      return;
    }
  ...
  async function handleDelete() {
    const confirmed = window.confirm(`Usunąć kod ${coupon.couponCode}? Tej operacji nie można cofnąć.`);
    if (!confirmed) return;
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 px-4 py-6">
          <form onSubmit={handleSave} className="max-h-[calc(100vh-48px)] w-full max-w-2xl overflow-y-auto rounded-xl border border-border bg-white p-5 shadow-card">
            <div className="flex items-start justify-between gap-4 border-b border-border pb-4">
              <div>
                <h2 className="text-xl font-black">Edytuj kod Udemy</h2>
                <p className="mt-1 text-sm text-slate-600">{coupon.courseId} / {coupon.couponCode}</p>
              </div>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <Field label="Course ID">
                <input
                  ...
                  onChange={(event) => setFormValue((value) => ({ ...value, courseId: event.target.value }))}
              ...
              <Field label="Locale">
                <select
                  ...
                  onChange={(event) => setFormValue((value) => ({ ...value, locale: event.target.value }))}
                  className="focus-ring h-11 w-full rounded-[10px] border border-border px-3 text-sm"
                >
              ...
              <Field label="Tytuł kursu">
                <input
                  ...
                  onChange={(event) => setFormValue((value) => ({ ...value, courseTitle: event.target.value }))}
                  className="focus-ring h-11 w-full rounded-[10px] border border-border px-3 text-sm"
                />
              </Field>
              <Field label="Kod kuponu">
                <input
                  ...
                  onChange={(event) => setFormValue((value) => ({ ...value, couponCode: event.target.value }))}
                  className="focus-ring h-11 w-full rounded-[10px] border border-border px-3 font-mono text-sm"
                  required
                />
              </Field>
              ...
              <label className="flex items-center gap-3 self-end rounded-[10px] border border-border px-3 py-3 text-sm font-semibold">
                <input
                  type="checkbox"
              ...
              <div className="sm:col-span-2">
                <Field label="URL Udemy">
                  <input
                    type="url"
                    value={formValue.udemyUrl}
                    onChange={(event) => setFormValue((value) => ({ ...value, udemyUrl: event.target.value }))}
                    className="focus-ring h-11 w-full rounded-[10px] border border-border px-3 text-sm"
                    required
                  />
                </Field>
